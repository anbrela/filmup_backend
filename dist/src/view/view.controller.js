"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ViewController = void 0;
const common_1 = require("@nestjs/common");
const view_service_1 = require("./view.service");
const viewMovie_dto_1 = require("./dto/viewMovie.dto");
const authGuard_1 = require("../auth/authGuard");
let ViewController = exports.ViewController = class ViewController {
    constructor(viewService) {
        this.viewService = viewService;
    }
    async viewMovie(viewMovieDto) {
        return this.viewService.viewMovie(viewMovieDto);
    }
};
__decorate([
    (0, common_1.Post)(),
    (0, common_1.UseGuards)(authGuard_1.AuthGuard),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [viewMovie_dto_1.ViewMovieDto]),
    __metadata("design:returntype", Promise)
], ViewController.prototype, "viewMovie", null);
exports.ViewController = ViewController = __decorate([
    (0, common_1.Controller)('view'),
    __metadata("design:paramtypes", [view_service_1.ViewService])
], ViewController);
//# sourceMappingURL=view.controller.js.map